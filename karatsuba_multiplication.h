...
__int128_t karatsuba_multiplication(__int128_t base, __int128_t first, __int128_t second){
  ...
  if(first<=base || second<=base) return second*first;

    //Caso geral: aplicação recursiva do algoritmo de redução de Karatsuba
  ...
  //Procedimentos
    ...
    split_number(&low2, &high2, second, B); //Operações de precisão múltipla
